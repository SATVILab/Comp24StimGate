...
1. **Bootstrap/setup**
   - After `renv::restore()`, run `Rscript -e 'projr::projr_install()'` if prompted to link project directories defined in `_projr.yml`. This only needs to be done once per machine.
2. **Build notebooks**
   ...
   - Precondition: `_data_raw` and other directories referenced in `_projr.yml` exist (they may be empty). If they are missing, create them to avoid `projr_path_get_dir` errors.
   - Postcondition: fresh outputs populate `_tmp/`, `_output/`, and `docs/Simulation.html`; the `manifest.csv` file updates accordingly.
3. **Targets debugging**
...
5. **Cleaning**
   - Use `Rscript -e 'projr::projr_clean()'` to purge `_tmp/` caches if simulations must be recomputed from scratch.
