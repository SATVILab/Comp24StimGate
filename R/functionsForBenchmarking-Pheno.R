...
genClusterCentersWithLabels <- function(
    possibleCenterMat,
    nPop = 5,
    seedVal = 0
) {
...
simSample <- function(
    ...
    if (isKnockout) {
        ...
        probVec[modProbLookups] <- (probVec[modProbLookups] +
            (targetProb / length(modProbLookups)))
    ...
    for (clusterNumber in seq(numClusters)) {
        ...
        if (mixtureType == "tPlusGauss") {
            if ((clusterNumber %% 2) == 0) {
                currentSample <- mvrnorm(
                    sampleSizeVec[clusterNumber],
                    mu = currentMu,
                    Sigma = currentSigma
                )
            } else {
        ...
        outData <- rbind(
            outData,
            t(apply(currentSample, 1, transformationType))
        )
    }
    colnames(outData) <- paste0("V", seq(ncol(outData)))
    outList <- list(
        sampleMatrix = outData,
        sampleLabels = outLabel,
...
safeModForPerfectDisc <- function(dataSet) {
    out <- tryCatch(
        {
            m <- glmer(
                cbind(childCount, (parentCount - childCount)) ~ resType +
                    (1 | subjectID),
                data = dataSet,
                family = "binomial",
...
simulateExperiment <- function(
    ...
    while (currentSample <= numSamples) {
        ...
        knockoutList <- append(
            knockoutList,
            list(table(sampleData$knockoutInfo))
        )
