...
This permutation test yields a p-value that determines whether these clusters are slated to merge or remain separate.
The significance threshold used can be adjusted using the `alpha` parameter. We recommend using the default value of `alpha = 0.05` with Bonferroni multiple comparison correction.
For a more conservative approach, the `alpha` value could be decreased to 0.01 or 0.001.
...
Please ensure that the relevant assay matrix includes both row and column names.
